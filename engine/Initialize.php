...
use Image_SEO_Generator\Engine;

/**
 * Image_SEO_Generator Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'image_seo_generator_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'image_seo_generator_initialize_failed', $err );
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'Image_SEO_Generator\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Image_SEO_Generator\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'Image SEO Generator is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', ISG_TEXTDOMAIN ) );
			}
